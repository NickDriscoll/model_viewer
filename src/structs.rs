...
impl Mesh {
    pub fn new(
        vao: GLuint,
        model_matrix: glm::TMat4<f32>,
        texture: GLuint,
        geo_boundaries: Vec<GLsizei>,
        materials: Option<Vec<Option<mtl::Material>>>,
    ) -> Self {
        Mesh {
            vao,
            geo_boundaries,
...
impl Controllers {
    ...
    pub fn pressed_this_frame(&self, controller_index: usize, flag: u32) -> bool {
        if let (Some(state), Some(p_state)) = (
...
impl<'a> RenderContext<'a> {
    pub fn new(
        p_matrices: &'a [glm::TMat4<f32>],
        v_matrices: &'a [glm::TMat4<f32>],
        light_direction: &'a glm::TVec4<f32>,
        shadow_map: GLuint,
        shadow_vp: &'a glm::TMat4<f32>,
        is_lighting: bool,
    ) -> Self {
...
impl Terrain {
    pub fn height_at(&self, xpos: f32, zpos: f32) -> f32 {
        self.amplitude
            * self.simplex.get([
                xpos as f64 * self.simplex_scale / self.scale as f64,
                zpos as f64 * self.simplex_scale / self.scale as f64,
            ]) as f32
    }
}
