use crate::{glm, routines, structs::ImageData};
...
pub unsafe fn compile_program_from_files(vertex_name: &str, fragment_name: &str) -> GLuint {
    let vertexshader = compile_shader_from_file(
        gl::VERTEX_SHADER,
        &format!("{}/vertex/{}", SHADER_PATH, vertex_name),
    );
    let fragmentshader = compile_shader_from_file(
        gl::FRAGMENT_SHADER,
        &format!("{}/fragment/{}", SHADER_PATH, fragment_name),
    );
...
pub unsafe fn submit_to_hmd(
    eye: Option<Eye>,
    openvr_compositor: &Option<Compositor>,
    target_handle: &openvr::compositor::texture::Texture,
) {
    if let (Some(ref comp), Some(e)) = (openvr_compositor, eye) {
        routines::handle_result(comp.submit(e, target_handle, None, None));
    }
}
...
pub unsafe fn create_vertex_array_object(
    vertices: &[f32],
    indices: &[u16],
    attribute_strides: &[i32],
) -> GLuint {
    let mut vao = 0;
...
pub unsafe fn load_texture_from_data(
...
pub unsafe fn bind_matrix4(program: GLuint, name: &str, matrix: &glm::TMat4<f32>) {
    gl::UniformMatrix4fv(
        ...
        1,
        gl::FALSE,
        &routines::flatten_glm(matrix) as *const GLfloat,
    );
}

pub unsafe fn bind_uniforms(
    program: GLuint,
    mat_uniforms: &[&str],
    mats: &[&glm::TMat4<f32>],
    vec_uniforms: &[&str],
    ...
    for i in 0..vec_uniforms.len() {
        gl::Uniform4fv(
            uniform_location(program, vec_uniforms[i]),
            1,
            &[vecs[i].x, vecs[i].y, vecs[i].z, vecs[i].w] as *const GLfloat,
        );
    }
...
pub unsafe fn bind_material(program: GLuint, material: &Option<mtl::Material>) {
    ...
    let (colors, specular_coefficient) = match material {
        Some(material) => {
            let amb = [
                material.color_ambient.r as f32,
                material.color_ambient.g as f32,
                material.color_ambient.b as f32,
            ];
            let diff = [
            ...
            let spec = [
                material.color_specular.r as f32,
                material.color_specular.g as f32,
                material.color_specular.b as f32,
            ];
    ...
    for i in 0..NAMES.len() {
        gl::Uniform3fv(
            uniform_location(program, NAMES[i]),
            1,
            &colors[i] as *const GLfloat,
        );
    }
    gl::Uniform1f(
        uniform_location(program, "specular_coefficient"),
        specular_coefficient,
    );
}

//Binds "buffer" to the next available attribute of vao as an array of mat4
pub unsafe fn bind_instanced_matrices(
    vao: GLuint,
    ...
    gl::BufferData(
        gl::ARRAY_BUFFER,
        (count * 16 * mem::size_of::<GLfloat>()) as GLsizeiptr,
    ...
    for i in 0..4 {
        ...
        gl::VertexAttribPointer(
            ...
            (4 * i * mem::size_of::<GLfloat>()) as *const c_void,
        );
